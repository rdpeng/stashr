...
%%\VignetteIndexEntry{The filehashRemote Package}
%%\VignetteDepends{filehash,tools,filehashRemote}
...
using the \textbf{filehashRemote} package for R}
...
\begin{abstract}
The \pkg{filehashRemote} package for R interfaces with a 
simple key-value style database where character string keys are 
associated with data values. The key-value databases can be either
stored locally on the user's computer or accessed remotely via the Internet.
Methods specific to the \pkg{filehashRemote} package allow users to create 
local data repositories or access previously created remote data repositories and insert, 
retrieve or delete data from the database as well as synchronize local
copies of the data to the remote version of the database. The 
'filehashRemote' and 'filehashLocal' objects in R allow a user to efficiently 
access information from a database by downloading only the data files indexed 
by user-specified keys. The local and remote counterparts of the \pkg{filehashRemote}
offer the potential to enhance reproducible research by allowing users of the \pkg{Sweave}
package in R to cache their R computations for a research paper in a 'filehashLocal' database. 
This database would then be stored on the Internet as a 'filehashRemote' database. When 
readers of the research paper wish to rerun the computations involved in creating a 
specific figure or calculating a specific numeric value, they can access the
'filehashRemote' database and obtain the R objects involved in the computation.
\end{abstract}
...
limitations in physical memory size. The \pkg{filehashRemote} R package extends the 
\pkg{filehash} package to local and remote databases. As is, the package 
\pkg{filehashRemote} can be used to create a local `filehashLocal'
key-value database where data files are indexed by character string keys. The
\pkg{filehashRemote} package can also be used by individuals to download
data from a `filehashRemote' key-value database stored remotely on the 
internet. This dual functionality of the \pkg{filehashRemote} package
...
The \pkg{filehashRemote} package is designed for interacting with
both local and remote data repositories. Each of the main user
interface functions in \pkg{filehashRemote} is a generic function 
with specific methods defined for repository objects of class
`filehashLocal', the local version of a key-value database, and for
objects of class `filehashRemote', the remote version of a key-value
database. When interacting with a `filehashLocal' data repository, 
the user can insert, fetch, delete and list keys of the available 
data files. When interacting with a `filehashRemote' data 
...
user interface functions for the `filehashRemote'repository 
are similar to those for the 'filehashLocal' repository. When 
...
previously been downloaded. The \pkg{filehashRemote} package also
...
The \pkg{filehashRemote} package allows users
to cache or access cached data in a `filehashLocal' or
`filehashRemote' repository. A key feature of the
\pkg{filehashRemote} package, is the ability for a user to download
desired data from a 'filehashRemote' repository in a local directory
and, at a later date, synchronize their locally cached data to the
data in the remote `filehashRemote' data repository. The synchronization
...
dramatically. Sychronization in the package \pkg{filehashRemote} is acheived 
...
\subsection{Creating a Local filehashRemote database}

There are two steps to creating a `filehashLocal' or a `filehashRemote'
object. The first step is to call \code{new("filehashLocal", dir = dir, name = name)} 
or \code{new("filehashRemote", dir = dir, url = url, name = name)} where 
`dir' is a character string specifying the local directory in which to 
create the new `filehashLocal' repository of the local copy of the 
`filehashRemote' repository. The `url' argument is unique to objects of the 
`filehashRemote' class and it specifies as a character string the url 
...
the `filehashLocal' or `filehashRemote' repository.  

The next step in creating a `filehashLocal' or a `filehashRemote' 
database is to call the \code{dbCreate} function.  The function \code{dbCreate} 
requires only the `filehashLocal' or `filehashRemote' object as an argument. The
information for where to build the local repository is stored in the `dir' slot
of the `filehashLocal' or `filehashRemote' object.


\subsection{Accessing a filehashRemote database}

The user-end interfaces to `filehashLocal' or `filehashRemote' databases 
...
`filehashRemote' and `filehashLocal' classes (with the exception of \code{dbSync}, 
as we shall see in Section~\ref{synch-interface}). The first argument for any of the 
above functions is an object of class `filehashRemote' or `filehashLocal'.

\subsubsection{\code{dbFetch}}

The function \code{dbFetch(db = "filehashLocal" or "filehashRemote", 
key = "character")} takes two arguments. The first argument is either a 
`filehashLocal' or `filehashRemote' object. The second argument is a 
character string key indexing a data file. 

For objects of the class `filehashRemote', \code{dbFetch} first checks to 
see if the provided key's data file and .SIG file exist 
in the local copy of the `filehashRemote' repository. If the data and .SIG files
...
Similarly, for objects of the class `filehashLocal', \code{dbFetch}
...
The function \code{dbInsert(db = "filehashLocal" or
"filehashRemote", key = "character", value= "ANY", overwrite = TRUE)} 
takes four arguments. The first argument, like any of the other user-end
interfaces is either a `filehashLocal' or `filehashRemote' object.
...
Calling \code{dbInsert} on a `filehashRemote' object returns an
...
On the other hand, calling \code{dbInsert} on a `filehashLocal' object 
...
The function \code{dbList(db = "filehashLocal")} or 
\code{dbList(db = "filehashRemote", save = FALSE)} takes a `filehashLocal' 
or `filehashRemote' object as its argument. For `filehashRemote' objects, 
...
repository. The function \code{dbExists(db = "filehashLocal" or 
"filehashRemote", key = key)} has a second argument, `key', which takes
...
For both objects of class `filehashRemote' and objects of class `filehashLocal', 
...
\code{dbDelete(db = "filehashRemote" or "filehashLocal", key = "character")}. 
Calling \code{dbDelete} on a `filehashRemote' object returns an error message 
since the user does not have access to the remote repository to delete the 
specified files. On the other hand, calling \code{dbDelete} on a `filehashLocal' 
...
The \pkg{filehashRemote} function for synchronizing local copies
of data stored on a remote repository is the generic function
\code{dbSync}. Currently, \code{dbSync} only has a method for
objects of the `filehashRemote' class because one would only need
to synchronize a local copy of a remote database. The
\code{dbSync} function takes as arguments a `filehashRemote'
...
\subsubsection{Objects of the class `filehashLocal'} 

For objects of the class `filehashLocal', we start out by 
defining a local directory in which we will create the 
repository.

<<define directory>>=
wd <- getwd()
dir <- file.path(wd,"filehashLocalExample")
@ 

\noindent Next, we perform a two-step process to create the `filehashLocal' object, 
which we will call `fhLocal'.
<<initialize filehashLocal>>=
fhLocal <- new("filehashLocal", dir = dir, name = "filehashLocal Example")
dbCreate(fhLocal)
@

\noindent We now insert different types of R objects into the local repository to 
create a basic `filehashLocal' database. Note that each time we call 
...
\subsubsection{Objects of the class `filehashRemote'} 

The same data used in the previous example for `filehashLocal'
has been stored in a `filehashRemote' repository on the internet at:
<<url>>=
myurl <- "http://www.biostat.jhsph.edu/~seckel/filehashRemoteExample/"
@

In this example, we will use the `filehashRemote' methods for 
the \pkg{filehashRemote} package interface functions: \code{dbFetch}, 
...
simply return error messages for `filehashRemote' objects.  

Again, we start off with the two-step process of creating a `filehashRemote' 
object. The local copy of the database will be located in our working
directory under a directory called `filehashRemoteExample'. 

<<create filehashRemote>>=
wd <- getwd()
dir <- file.path(wd,"filehashRemoteExample") 
fhRemote <- new("filehashRemote", url= myurl, 
            dir = dir, name= "filehashRemote Example")
dbCreate(fhRemote)
@

\noindent When we run \code{dbList} on the `filehashRemote' object, `fhRemote',
...
\begin{verbatim}
> dbFetch(fhRemote, "vector") 
trying URL 'http://www.biostat.jhsph.edu/~seckel/filehashRemoteExample//data/vector'
Content type 'text/plain; charset=UTF-8' length 59 bytes
opened URL
downloaded 59 bytes

trying URL 'http://www.biostat.jhsph.edu/~seckel/filehashRemoteExample//data/vector.SIG'
...
\begin{verbatim}
> dbFetch(fhRemote, "matrix") 
trying URL 'http://www.biostat.jhsph.edu/~seckel/filehashRemoteExample//data/matrix'
Content type 'text/plain; charset=UTF-8' length 97 bytes
opened URL
downloaded 97 bytes

trying URL 'http://www.biostat.jhsph.edu/~seckel/filehashRemoteExample//data/matrix.SIG'
Content type 'text/plain; charset=UTF-8' length 42 bytes
opened URL
downloaded 42 bytes
...
The \pkg{filehashRemote} package has been designed as a
...
of \code{Sweave} documents to a filehashLocal database. This local 
database would then need to be transferred in an automatic way as a
filehashRemote database to a repository on the internet. Ideally,
...
(stored in the filehashRemote database) used in the computation of
